...
- Use showContact when a user asks how to get in touch.`,L={formatRequest(l,e){return{...T.formatRequest(l,e),stream:!1}},parseResponse(l){var e,t;return{content:l.content??"",stopReason:(e=l.toolCalls)!=null&&e.length?"tool_use":"stop",toolCalls:((t=l.toolCalls)==null?void 0:t.map(s=>({id:s.id,name:s.function.name,args:JSON.parse(s.function.arguments)})))??[]}}},A=`
  :host { display: block; }
  .trigger {
    display: flex;
    align-items: center;
...
`,q=`
  ...
  .overlay {
    ...
    padding: 1rem;
    /* dvh updates when the virtual keyboard appears on Android/newer iOS;
       --kb-vh is set from the visualViewport API as a fallback for browsers
       (older iOS Safari) that don't resize dvh for the keyboard. */
  ...
  .window {
    ...
    width: 100%;
    max-width: 480px;
    height: min(600px, calc(var(--kb-vh, 100dvh) * 0.8));
...
`,I='<svg viewBox="0 0 24 24"><path d="M2.01 21L23 12 2.01 3 2 10l15 2-15 2z"/></svg>',R='<svg viewBox="0 0 20 20"><path d="M10 2a.75.75 0 0 1 .75.75v1.5a.75.75 0 0 1-1.5 0v-1.5A.75.75 0 0 1 10 2ZM10 15a.75.75 0 0 1 .75.75v1.5a.75.75 0 0 1-1.5 0v-1.5A.75.75 0 0 1 10 15ZM10 7a3 3 0 1 0 0 6 3 3 0 0 0 0-6Z"/></svg>',z=`
...
`;class V extends HTMLElement{connectedCallback(){var t,s;this._backendUrl=this.getAttribute("backend-url")||"https://backend-utj7y3c5ba-uc.a.run.app",this._manager=null,this._loading=!1,this._model="gpt-4o-mini";const e=this.attachShadow({mode:"open"});e.innerHTML=`<style>${A}</style>
      <button class="trigger" aria-label="Open AI chat">${R} Ask me anything</button>`,e.querySelector(".trigger").addEventListener("click",()=>this._open()),this._portalHost=document.createElement("div"),document.body.appendChild(this._portalHost),this._portal=this._portalHost.attachShadow({mode:"open"}),this._portal.innerHTML=`<style>${q}</style>${z}`,this._portal.querySelector(".btn-close").addEventListener("click",()=>this._close()),this._portal.querySelector(".btn-clear").addEventListener("click",()=>this._clearConversation()),this._portal.querySelector(".overlay").addEventListener("click",o=>{o.target===o.currentTarget&&this._close()}),this._portal.querySelector(".model-select").addEventListener("change",o=>{this._model=o.target.value,this._manager&&(this._manager._config.model=this._model)}),this._portal.querySelector(".send").addEventListener("click",()=>this._send()),this._portal.querySelector(".input").addEventListener("keydown",o=>{o.key==="Enter"&&!o.shiftKey&&(o.preventDefault(),this._send())}),document.addEventListener("keydown",o=>{o.key==="Escape"&&this._close()}),this._onViewportChange=()=>{const o=window.visualViewport;o&&this._portal.host.style.setProperty("--kb-vh",`${o.height}px`)},(t=window.visualViewport)==null||t.addEventListener("resize",this._onViewportChange),(s=window.visualViewport)==null||s.addEventListener("scroll",this._onViewportChange),this._portal.querySelector(".input").addEventListener("focus",()=>{setTimeout(()=>{this._portal.querySelector(".input").scrollIntoView({block:"nearest"})},300)})}disconnectedCallback(){var e,t,s;(e=window.visualViewport)==null||e.removeEventListener("resize",this._onViewportChange),(t=window.visualViewport)==null||t.removeEventListener("scroll",this._onViewportChange),(s=this._portalHost)==null||s.remove()}_getManager(){return this._manager?this._manager:(this._manager=new E({endpoint:`${this._backendUrl}/api/chat`,model:this._model,hooks:L}),this._manager.addTool("createNavigation",{description:"Renders a clickable navigation button in the chat. Use when the user asks about a page or wants to go somewhere on the site.",parameters:{type:"object",properties:{label:{type:"string",description:'Button label, e.g. "View Projects"'},url:{type:"string",description:'Relative URL, e.g. "/projects.html"'}},required:["label","url"]}},({label:e,url:t})=>(this._appendAction("nav",e,t),{success:!0})),this._manager.addTool("createDownload",{description:"Renders a download button for the resume/CV in the chat.",parameters:{type:"object",properties:{},required:[]}},()=>(this._appendAction("download","Download Resume","/assets/ben-bernardy-resume.pdf",!0),{success:!0})),this._manager.addTool("scrollToSection",{description:"Smoothly scrolls the page to a section. Available sections: bento-bio, bento-ai, tools, bento-listening.",parameters:{type:"object",properties:{sectionId:{type:"string",description:"The element ID to scroll to"}},required:["sectionId"]}},({sectionId:e})=>{const t=document.getElementById(e);return t?(t.scrollIntoView({behavior:"smooth",block:"center"}),{success:!0}):{success:!1,error:"Section not found"}}),this._manager.addTool("showContact",{description:"Renders clickable contact links in the chat. Use when the user asks how to reach Ben or get in touch.",parameters:{type:"object",properties:{},required:[]}},()=>(this._appendContact(),{success:!0})),this._manager.on("tool:call",({name:e})=>{this._appendToolCall(e)}),this._manager)}_open(){this._portal.querySelector(".overlay").removeAttribute("hidden"),this._onViewportChange(),document.body.style.overflow="hidden",this._portal.querySelector(".input").focus()}_close(){this._portal.querySelector(".overlay").setAttribute("hidden",""),document.body.style.overflow=""}async _send(){const e=this._portal.querySelector(".input"),t=e.value.trim();if(!(!t||this._loading)){e.value="",this._appendMessage("user",t),this._setLoading(!0);try{const s=await this._getManager().send(t,{system:S});this._appendMessage("assistant",s.content)}catch(s){this._appendMessage("error",this._errorMessage(s))}finally{this._setLoading(!1),this._portal.querySelector(".input").focus()}}}_appendToolCall(e){const t=this._portal.querySelector(".messages"),s=document.createElement("div");s.className="message message--tool",s.innerHTML=`<svg viewBox="0 0 24 24"><path d="M22.7 19l-9.1-9.1c.9-2.3.4-5-1.5-6.9-2-2-5-2.4-7.4-1.3L9 6 6 9 1.6 4.7C.4 7.1.9 10.1 2.9 12.1c1.9 1.9 4.6 2.4 6.9 1.5l9.1 9.1c.4.4 1 .4 1.4 0l2.3-2.3c.5-.4.5-1.1.1-1.4z"/></svg>calling tool: <strong>${e}</strong>`,t.appendChild(s),this._scrollToEnd(t)}_errorMessage(e){var o;const t=(e==null?void 0:e.message)??"";if(!t||t.includes("Failed to fetch")||t.includes("NetworkError"))return"Unable to reach the server — please check your connection and try again.";const s=(o=t.match(/HTTP (\d+)/))==null?void 0:o[1];return s==="429"?"Rate limit reached — please wait a moment before sending another message.":s==="502"?"The AI provider returned an error. Please try again shortly.":s>="500"?"A server error occurred. Please try again.":s==="401"?(fetch(`${this._backendUrl}/api/notify`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({reason:"Chat 401 — invalid API key",detail:"OpenAI returned 401 for a chat request on the portfolio."})}).catch(()=>{}),"There's an authentication issue with the AI. An email has been sent to Ben so he can fix it — sorry for the inconvenience!"):"Something went wrong. Please try again."}_clearMessages(){this._portal.querySelector(".messages").innerHTML=""}_clearConversation(){var e;(e=this._manager)==null||e.reset(),this._clearMessages()}_appendAction(e,t,s,o=!1){const n=this._portal.querySelector(".messages"),r=document.createElement("div");r.className="message message--action";const i=e==="download"?'<svg viewBox="0 0 24 24"><path d="M19 9h-4V3H9v6H5l7 7 7-7zM5 18v2h14v-2H5z"/></svg>':'<svg viewBox="0 0 24 24"><path d="M10 6v2H5v11h11v-5h2v7H3V6h7zm11-3v8l-3-3-6 6-1.5-1.5 6-6L13 3h8z"/></svg>',a=document.createElement("a");a.className=`action-btn action-btn--${e}`,a.href=s,a.innerHTML=`${i}${t}`,o?a.download="":a.addEventListener("click",()=>this._close()),r.appendChild(a),n.appendChild(r),this._scrollToEnd(n)}_appendContact(){const e=this._portal.querySelector(".messages"),t=document.createElement("div");t.className="message message--action";const s=[{label:"Email",href:"mailto:benbernardy@gmail.com",icon:'<svg viewBox="0 0 24 24"><path d="M20 4H4c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2zm0 4-8 5-8-5V6l8 5 8-5v2z"/></svg>'},{label:"GitHub",href:"https://github.com/BenKhz",icon:'<svg viewBox="0 0 24 24"><path d="M12 2A10 10 0 0 0 2 12c0 4.42 2.87 8.17 6.84 9.5.5.08.66-.23.66-.5v-1.69c-2.77.6-3.36-1.34-3.36-1.34-.46-1.16-1.11-1.47-1.11-1.47-.91-.62.07-.6.07-.6 1 .07 1.53 1.03 1.53 1.03.87 1.52 2.34 1.07 2.91.83.09-.65.35-1.09.63-1.34-2.22-.25-4.55-1.11-4.55-4.92 0-1.11.38-2 1.03-2.71-.1-.25-.45-1.29.1-2.64 0 0 .84-.27 2.75 1.02.79-.22 1.65-.33 2.5-.33.85 0 1.71.11 2.5.33 1.91-1.29 2.75-1.02 2.75-1.02.55 1.35.2 2.39.1 2.64.65.71 1.03 1.6 1.03 2.71 0 3.82-2.34 4.66-4.57 4.91.36.31.69.92.69 1.85V21c0 .27.16.59.67.5C19.14 20.16 22 16.42 22 12A10 10 0 0 0 12 2z"/></svg>'},{label:"LinkedIn",href:"https://linkedin.com/in/ben-bernardy",icon:'<svg viewBox="0 0 24 24"><path d="M19 3a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h14m-.5 15.5v-5.3a3.26 3.26 0 0 0-3.26-3.26c-.85 0-1.84.52-2.32 1.3v-1.11h-2.79v8.37h2.79v-4.93c0-.77.62-1.4 1.39-1.4a1.4 1.4 0 0 1 1.4 1.4v4.93h2.79M6.88 8.56a1.68 1.68 0 0 0 1.68-1.68c0-.93-.75-1.69-1.68-1.69a1.69 1.69 0 0 0-1.69 1.69c0 .93.76 1.68 1.69 1.68m1.39 9.94v-8.37H5.5v8.37h2.77z"/></svg>'}],o=document.createElement("div");o.className="contact-row",s.forEach(({label:n,href:r,icon:i})=>{const a=document.createElement("a");a.className="contact-chip",a.href=r,a.target="_blank",a.rel="noopener noreferrer",a.innerHTML=`${i}${n}`,o.appendChild(a)}),t.appendChild(o),e.appendChild(t),this._scrollToEnd(e)}_scrollToEnd(e){e.scrollTo({top:e.scrollHeight,behavior:"smooth"})}_appendMessage(e,t){const s=this._portal.querySelector(".messages"),o=document.createElement("div");o.className=`message message--${e}`,o.textContent=t,s.appendChild(o),this._scrollToEnd(s)}_setLoading(e){this._loading=e,this._portal.querySelector(".send").disabled=e;const t=this._portal.querySelector(".typing");if(e&&!t){const s=document.createElement("div");s.className="message message--assistant typing",s.innerHTML="<span></span><span></span><span></span>";const o=this._portal.querySelector(".messages");o.appendChild(s),this._scrollToEnd(o)}else!e&&t&&t.remove()}}customElements.define("bento-chat-agent",V);
