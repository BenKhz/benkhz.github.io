import{i as _}from"./init-3UIJpjAr.js";import{_ as x}from"./preload-helper-D7HrI6pR.js";const y=`
  :host {
    display: block;
    position: relative;
    width: 100%;
...
`,b=(o,t)=>o+Math.random()*(t-o);class M extends HTMLElement{connectedCallback(){const t=this.attachShadow({mode:"open"});t.innerHTML=`<style>${z}</style><slot></slot>`,this._shadow=t,requestAnimationFrame(()=>this._build())}_build(){const t=this.offsetWidth||300,e=this.offsetHeight||300,i=72,s=4,a=t/s,h=e/Math.ceil(v.length/s);v.forEach((l,p)=>{const u=p%s,d=Math.floor(p/s),r=u*a+b(a*.1,a*.9)-i/2,g=d*h+b(h*.1,h*.9)-i/2,c=b(-22,22),n=Math.floor(b(1,6)),m=document.createElement("div");m.className="tile",m.setAttribute("aria-label",l.name),m.style.cssText=`left:${r}px;top:${g}px;transform:rotate(${c}deg);z-index:${n}`,m.innerHTML=`<iconify-icon icon="${l.icon}" aria-hidden="true"></iconify-icon>`,this._shadow.appendChild(m)})}disconnectedCallback(){}}customElements.define("tech-tiles",M);const C=`
  :host {
    display: contents;
  }
  .playback-accent {
...
`;function H(o){const t=Math.floor((Date.now()-new Date(o))/6e4);if(t<1)return"just now";if(t<60)return`${t}m ago`;const e=Math.floor(t/60);return e<24?`${e}h ago`:`${Math.floor(e/24)}d ago`}class I extends HTMLElement{connectedCallback(){const t=this.attachShadow({mode:"open"});t.innerHTML=A+`<style>${C}</style>`,this._shadow=t,this._hydrate()}async _hydrate(){const t=await this._fetchRecentTracks();if(!(t!=null&&t.length))return;const[e]=t,i=this.querySelector(".playback-title"),s=this.querySelector(".playback-label"),a=this.querySelector(".playback-tracks"),h=this._shadow.querySelector(".playback-image"),l=this._shadow.querySelector(".playback-preview");if(i&&(i.textContent=e.name,e.explicit)){const n=document.createElement("span");n.className="playback-explicit",n.textContent="E",i.append(n)}const p=document.createElement("p");if(p.className="playback-album",p.textContent=`${e.artists[0]} · ${e.album.name}`,i==null||i.after(p),s&&(s.textContent=H(e.playedAt)),a==null||a.remove(),h&&e.album.image&&(h.style.backgroundImage=`url('${e.album.image}')`),l&&e.previewUrl){const n=new Audio(e.previewUrl);l.addEventListener("click",()=>{n.paused?(n.play(),l.innerHTML=$):(n.pause(),l.innerHTML=f)}),n.addEventListener("ended",()=>{l.innerHTML=f})}else l&&l.remove();const u=this._shadow.querySelector(".playback-wave"),d=400,r=Math.random()*.6+.1,g=Math.round(r*d),c=Math.round((1-r)*(e.durationMs/1e3))*3;u.style.setProperty("--wave-from",`${g}px`),u.style.setProperty("--wave-to",`${d}px`),u.style.setProperty("--playback-duration",`${c}s`)}async _fetchRecentTracks(){try{return(await(await fetch("https://backend-utj7y3c5ba-uc.a.run.app/api/spotify/recent?limit=1")).json()).tracks??null}catch{return null}}}customElements.define("bento-listening",I);const P=`
  :host {
    display: contents;
  }
...
`;class q extends HTMLElement{connectedCallback(){const t=this.attachShadow({mode:"open"});t.innerHTML=j+`<style>${P}</style>`}}customElements.define("bento-location",q);_(()=>{const o=document.getElementById("bento-ai");if(!o)return;const t=new IntersectionObserver(([e])=>{e.isIntersecting&&(t.disconnect(),x(()=>import("./bento-chat-agent-rhkP2Os_.js"),[]))},{threshold:.1});t.observe(o)});
