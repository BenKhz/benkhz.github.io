import{_ as x,i as y}from"./init-DFVXtHBA.js";const _=`
  :host {
    display: block;
    position: relative;
    width: 100%;
...
`,g=(a,t)=>a+Math.random()*(t-a);class M extends HTMLElement{connectedCallback(){const t=this.attachShadow({mode:"open"});t.innerHTML=`<style>${S}</style><slot></slot>`,this._shadow=t,requestAnimationFrame(()=>this._build())}_build(){const t=this.offsetWidth||300,e=this.offsetHeight||300,i=72,s=4,n=t/s,h=e/Math.ceil(v.length/s);v.forEach((l,p)=>{const u=p%s,d=Math.floor(p/s),r=u*n+g(n*.1,n*.9)-i/2,m=d*h+g(h*.1,h*.9)-i/2,c=g(-22,22),o=Math.floor(g(1,6)),b=document.createElement("div");b.className="tile",b.setAttribute("aria-label",l.name),b.style.cssText=`left:${r}px;top:${m}px;transform:rotate(${c}deg);z-index:${o}`,b.innerHTML=`<iconify-icon icon="${l.icon}" aria-hidden="true"></iconify-icon>`,this._shadow.appendChild(b)})}disconnectedCallback(){}}customElements.define("tech-tiles",M);const C=`
  :host {
    display: contents;
  }
  .playback-accent {
...
`;function H(a){const t=Math.floor((Date.now()-new Date(a))/6e4);if(t<1)return"just now";if(t<60)return`${t}m ago`;const e=Math.floor(t/60);return e<24?`${e}h ago`:`${Math.floor(e/24)}d ago`}class I extends HTMLElement{connectedCallback(){const t=this.attachShadow({mode:"open"});t.innerHTML=$+`<style>${C}</style>`,this._shadow=t,this._hydrate()}async _hydrate(){const t=await this._fetchRecentTracks();if(!(t!=null&&t.length))return;const[e]=t,i=this.querySelector(".playback-title"),s=this.querySelector(".playback-label"),n=this.querySelector(".playback-tracks"),h=this._shadow.querySelector(".playback-image"),l=this._shadow.querySelector(".playback-preview");if(i&&(i.textContent=e.name,e.explicit)){const o=document.createElement("span");o.className="playback-explicit",o.textContent="E",i.append(o)}const p=document.createElement("p");if(p.className="playback-album",p.textContent=`${e.artists[0]} · ${e.album.name}`,i==null||i.after(p),s&&(s.textContent=H(e.playedAt)),n==null||n.remove(),h&&e.album.image&&(h.style.backgroundImage=`url('${e.album.image}')`),l&&e.previewUrl){const o=new Audio(e.previewUrl);l.addEventListener("click",()=>{o.paused?(o.play(),l.innerHTML=A):(o.pause(),l.innerHTML=f)}),o.addEventListener("ended",()=>{l.innerHTML=f})}else l&&l.remove();const u=this._shadow.querySelector(".playback-wave"),d=400,r=Math.random()*.6+.1,m=Math.round(r*d),c=Math.round((1-r)*(e.durationMs/1e3))*3;u.style.setProperty("--wave-from",`${m}px`),u.style.setProperty("--wave-to",`${d}px`),u.style.setProperty("--playback-duration",`${c}s`)}async _fetchRecentTracks(){try{return(await(await fetch("https://backend-utj7y3c5ba-uc.a.run.app/api/spotify/recent?limit=1")).json()).tracks??null}catch{return null}}}customElements.define("bento-listening",I);const P=`
  :host {
    display: contents;
  }
  ...
  .map-wrap iframe {
    ...
    left: -20%;
    border: 0;
    /* Purely decorative — no dragging/zooming the embed, no click-through to Google Maps */
    pointer-events: none;
  }
...
`,q=`
  <div class="map-wrap">
    <iframe
      src="https://maps.google.com/maps?q=San+Diego,+CA&t=&z=12&ie=UTF8&iwloc=&output=embed"
      loading="lazy"
...
`;class j extends HTMLElement{connectedCallback(){const t=this.attachShadow({mode:"open"});t.innerHTML=q+`<style>${P}</style>`}}customElements.define("bento-location",j);y(()=>{const a=document.getElementById("bento-ai"),t=document.getElementById("chat-cta");if(!a)return;let e;const i=()=>e??(e=x(()=>import("./bento-chat-agent-CX52HEEg.js").then(n=>n.b),[])),s=new IntersectionObserver(([n])=>{n.isIntersecting&&(s.disconnect(),i())},{threshold:.1});s.observe(a),t==null||t.addEventListener("click",async()=>{var n;await i(),await customElements.whenDefined("bento-chat-agent"),(n=document.querySelector("bento-chat-agent"))==null||n.open()})});
